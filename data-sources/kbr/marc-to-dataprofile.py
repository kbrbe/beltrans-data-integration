...
def main():
  ...
  parser.add_option('-m', '--header-mapping-file', action='store', help='The CSV file containing a mapping from marc fields to headers for the CSV output file')
  (options, args) = parser.parse_args()

  #
  # Check if we got all required arguments
  #
  if( (not options.input_file) or (not options.output_file) or (not options.header_mapping_file) ):
    parser.print_help()
    exit(1)
  ...
  with open(options.output_file, 'w', encoding='utf-8') as outFile,
       open(options.header_mapping_file, 'r', encoding='utf-8') as headerFile:

    #
    # read the header mapping into a data structure for lookup
    #
    mappingReader = csv.reader(headerFile)
    fieldMapping = {rows[0]:rows[1] for rows in mappingReader}
    ...
    for event, elem in ET.iterparse(options.input_file, events=('start', 'end')):
      ...
      if  event == 'end' and elem.tag == ET.QName(NS_MARCSLIM, 'record'):
        counter += 1
        utils.getDataprofileRecordFromMARCXML(elem, fieldMapping)

        # todo, iterate over created dictionary and write selected fields with a CSV dict writer to the csv file
        elem.clear()
