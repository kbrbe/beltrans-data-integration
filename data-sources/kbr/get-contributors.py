...
def main():
  ...
  print("Found roles (within " + str(counter) + " records): ")
  print("Persons:")
  printRoleStats(roleStats['person'])
  print("Organizations:")
  printRoleStats(roleStats['org'])
...
def printRoleStats(stats):
  ...
  sortedValues = sorted(stats.items(), key=lambda x:x[1], reverse=True)
  for roleTuple in sortedValues:
    role = roleTuple[0]
    number = roleTuple[1]
    if role in markRelators:
      print("\t" + markRelators[role] + ": " + str(number))
    else:
      print("\tNon-valid MARC relator code '" + role + "': " + str(number))
