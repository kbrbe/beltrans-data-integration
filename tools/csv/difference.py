...
def main():
  ...
  parser.add_option('-c', '--column', action='store', help='The optional column name that should be taken, default is the first column')
  ...
  if options.column:
    # check if each input file contains the specified column
    for inputFilename in args:
      with open(inputFilename, 'r') as inFile:
        inputReader = csv.DictReader(inFile, delimiter=delimiter)
        print(f'check for {inputFilename} if {options.column} is in {inputReader.fieldnames}')
  ...
  for a in args:
    with open(a, 'r') as fIn:
      ...
      for row in reader:
        identifier = row[options.column] if options.column else row[fieldnames[0]]
  ...
  if(options.output_file):
    with open(options.output_file, 'w') as outFile:
      ...
      outputWriter.writerow([options.column])
