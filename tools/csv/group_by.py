...
def main():
  ...
  parser.add_argument('--id-column', action='store', help='The name of the column with unique identifiers')
  ...
  #
  if( (not options.output_file) or (not options.input_file) or (not options.column) or (not options.id_column)):
    parser.print_help()
    exit(1)
  ...
  with open(inputFilename, 'r') as fIn, \
    ...
    outputWriter = csv.DictWriter(fOut, fieldnames=[uniqueIDColumn] + columns + ['count'] + list(statsHeaders), restval='0')
    outputWriter.writeheader()

    sortedCount = dict( sorted(count.items(), key=operator.itemgetter(1), reverse=True) )
    for uniqueValue in sortedCount:
      outputRow = {uniqueIDColumn: uniqueValue}
