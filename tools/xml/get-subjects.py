...
import tools.utils
from tools.xml.predicate_object_filter import PredicateObjectConfigFilter
from tools.xml.predicate_object_filter import PredicateObjectLookupFilter
...
def checkArguments(parser, options, args):
  ...
  prefix, resource = options.subject_tag.split(':')
  if prefix not in ALL_NS:
    print(f'Unknown prefix {prefix}, should be one of {ALL_NS.keys()}')
    exit(1)
  ...
  with open(options.output_file, 'w') as outFile:
    ...
    for inputFile in inputFiles:
      ...
      if inputFile.endswith('.xml'):
        ...
        context = ET.iterparse(inputFilePath, events=('start', 'end'))
          ...
          if  event == 'end' and elem.tag == ET.QName(ALL_NS[subjectPrefix], subjectTerm):
            ...
            for predicate in predicates:
              ...
              for po in foundTriples:
                ...
                objectValue = po.attrib[RDF_RESOURCE] if RDF_RESOURCE in po.attrib else tools.utils.getElementValue(po)
